...
int main(int argc, char *argv[]) {
    ...
    while(true){
        ...
        cv::Mat forwardrgb = img_rgb;//前向RGB

        cv::resize(img_rgb, img_rgb, cv::Size(256, 192), 0.0, 0.0, cv::INTER_CUBIC);//cv::Size(320, 240)
        if (!model.infer(context, buffers, img_rgb, joints, maxval)){// 推理获取关节点坐标和置信度,关节点像素坐标保存于joints
        ...
        // std::cout << "主线程--捕获RGB图+推理耗时capture_time =  " <<dec<< capture_time*1000 << std::endl;
        ...
        cv::Mat depth_back(Size(WIDTH, HEIGHT), CV_16UC1, (void*)depth_frame.get_data(), Mat::AUTO_STEP);
        //后向RGB
        rs2::frame color_frame = frames.get_color_frame();
        cv::Mat backrgb(Size(WIDTH, HEIGHT), CV_8UC3, (void*)color_frame.get_data(), Mat::AUTO_STEP);
        //获取关节点三维坐标
        std::vector<cv::Point3f> joints_coord = humandetect::get_joints_coord_bydepth(joints,maxval,depth_forward);
        imageinfo imginfo(cycle_times,get_time_now(),forwardrgb,depth_forward,backrgb,depth_back,joints,joints_coord,maxval);
