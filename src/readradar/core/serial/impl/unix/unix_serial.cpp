...
// 通过tio设置串口
  ...
  set_databits(&tio, bytesize_);        // 数据位的大小
  set_parity(&tio, parity_);            // 奇偶校验的类型
  set_stopbits(&tio, stopbits_);        // 停止位的数量
  set_flowcontrol(&tio, flowcontrol_);  // 流控制

  if (!setTermios(&tio)) {            // 设置串口熟悉
    close();
    return false;
  }
...
bool Serial::SerialImpl::setStandardBaudRate(speed_t baudrate) {
  ...
  //  termios v2 版本的 ioctl() 函数 获取fd_ 的串口设备信息
  if (::ioctl(fd_, TCGETS2, &tio2) != -1) {
    // 如果检测到当前串口设备的波特率被设置为自定义值，就重新设置为标准波特率
